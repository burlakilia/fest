		...
		parser.onopentag = function (node) {
			...
			switch (node.local){
				...
				case 'each':
					...
					section.source += 'for(' + _getAttr(node, 'index') + ' in __fest_iterator' + counters.counter + '){ if (!__fest_iterator' + counters.counter + '.hasOwnProperty(' + _getAttr(node, 'index') + ')) {continue;}';
					if (node.attributes.value) {
