	...
	function _compile (data){
		...
		parser.onopentag = function (node) {
			...
			if (node.ns[node.prefix] != fest_ns){
				...
				if (isInI18n) {
					section.source += '<' + node.name + attrs.text + '>';
					return;
				}
				...
				case 'i18n':
					...
					(function () {
						...
						if (Object.keys(params).length) {
							section.source += ',' + JSON.stringify(params);
						}

						if (node.attributes.context) {
							section.source += ',' + '"' + node.attributes.context.value + '"';
						}
		...
		parser.ontext = parser.oncdata = function (text) {
			...
			} else if (stack[stack.length - 1] === 'i18n' || isInI18n) {
				section.source += text;
			...
			template =
				'function ' + name + '(__fest_context){' +
					...
					'var __fest_self=this,' +
						...
						'i18n=typeof __fest_i18n === "undefined" ? function () {__fest_log_error("Error: can\'t find __fest_i18n function in global scope.")} : __fest_i18n,' +
						'___fest_log_error;' +
